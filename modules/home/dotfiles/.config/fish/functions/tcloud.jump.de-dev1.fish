function tcloud.jump.de-dev1
asp kpn-iam ;set -x credentials (aws sts assume-role --role-arn arn:aws:iam::612446854122:role/kpn-developer --role-session-name containers --duration 28800); set -x access_key (echo $credentials | jq -r '.Credentials.AccessKeyId'); set -x secret_key (echo $credentials | jq -r '.Credentials.SecretAccessKey'); set -x session_token (echo $credentials | jq -r '.Credentials.SessionToken');
